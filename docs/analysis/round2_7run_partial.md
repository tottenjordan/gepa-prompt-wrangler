...
> **Salvaged 2026-09-08 from `chore/prep-model-sweep-manifests` (PR #5, closed
> unmerged).** Kept for the record; the branch was deleted. The code changes on
> that branch are all superseded — its `GOOGLE_API_KEY` strip in particular,
> because `deploy.py` now does not copy `.env` into the build package at all.
>
> **Read the numbers with the floor in mind.** These runs are N=1 with no
> dedicated control arm. The analyses do anchor a floor on runs that happened to
> return their seed unchanged, and land on |Δ| ≲ 0.029. Campaign 06 later
> measured it properly — four control arms, both publishers, 100% coverage — and
> got **~0.058 at n=1**, roughly twice as wide. Under that floor **every delta
> reported here falls inside the noise**, including the two called "robustly
> real" at the time. See [../doe/06-pipeline-noise-floor.md](../doe/06-pipeline-noise-floor.md).
>
> What survives is the *qualitative* finding, which the floor does not touch:
> the seed's value is model-specific and can be negative, and pro/lite look
> optimization-resistant at budget 150 from either seed.

> **Superseded by `round2_10run_matrix.md`**, which backfills the three
> blocked bare runs. It also cites `vertex_eval_service_incident.md`, which
> was **retracted** on that same branch — the degradation was our own
> `GOOGLE_API_KEY` leaking into the deployed env, not a Vertex fault. That
> retracted doc was deliberately not salvaged.
