...
| [repo-traps.md](repo-traps.md) | Non-obvious footguns: gitignored lockfile, duplicated config, why `GOOGLE_CLOUD_LOCATION` can't express the per-model location rule, Agent Registry services invisible to `gcloud run services list`, `TODO` goldens. Also **`GOOGLE_GENAI_USE_ENTERPRISE=1`** — set in `.env`, read by nothing here, and a second independent way of telling `google-genai` to use Vertex, so it 401s any API-key client that inherits this environment *even with* `GOOGLE_GENAI_USE_VERTEXAI` unset. That is the unnamed "something else" the 2026-08-21 PaperBanana entry left open. |
