...
empty — one ADK timeout (120s) before the error, i.e. exactly when the hung call started.
The cause is ADK's own `Runner.close()` tearing down a toolset that `agent.clone()`'s
shallow copy shares between candidates — **1,812 closes per stage**, against the ~131 the
removed refresh contributed. See
[../analysis/2026-09-12-mcp-toolset-loss-localised.md](../analysis/2026-09-12-mcp-toolset-loss-localised.md)
and `scripts/analyze_toolset_loss.py`.

**Read the sections below in order: they record three mechanisms in the order they were
believed, and two of them are wrong. The way they were wrong is the useful part.**
...
### Attempted 2026-09-10, insufficient — removing the refresh

**This did not fix it.** Campaign 08 ran with the refresh gone and lost 16 of 131
generations (12%), against 14% before. The reasoning below was right that the refresh was
unnecessary and wrong that it was the cause: it removed ~131 closes and left **1,812**.
Nobody counted the remainder. Kept because the argument reads as conclusive and was not.
...
### Reproduced locally 2026-09-10 — but the reproduction was the wrong shape

**Its negative result was about the reproduction, not about production.** It raced *one*
close against *one* `get_tools()`; production races a burst of ~32 closes against
concurrent calls from many runners sharing a single toolset. That is why it measured 0/18
at the production cache setting while production fails at 12%.
...
- **RETRACTED: "a `re-warmed N/N in 0.1s` line is a red flag".** Three localhost round
  trips genuinely take about that; three remote ones measured 0.76s. The timing said
  nothing either way, and there is no re-warm at all since PR #75.
...
### Fixed 2026-09-10 — snapshot before write, at submit time

`deploy_pipeline.submit()` now calls `artifact_snapshot.snapshot_prior_run()` **before**
anything writes: any existing `stages/` and `reports/` for that `run_id` are copied to
`pipeline-runs/archive/<run_id>/<stamp>/`, named for when the prior run last wrote. First
submission of a manifest finds nothing and costs nothing.

**Why not re-key the artifact paths**, which is the obvious fix and was the plan. Giving
each submission its own prefix means threading a per-submission value into the components
that write there, and KFP caches on component body hash *and input parameter values* — so
a value that changes every submission gives every component a cache miss forever. That
trades the artifacts for the cache, and the cache is worth ~7 hours of optimize on exactly
the resubmissions this bug affects. Snapshotting client-side touches **no component**: the
cache key is untouched, `components.py` is not modified, and the live prefix keeps meaning
"the most recent run".

Reading an archived run needs no new code. `campaign_floor.fetch_arms` builds
`pipeline-runs/{run_id}/stages/...`, so `wrangler floor archive/run-8a5905dee0/20260909T175209Z`
resolves. A test asserts the archived tree is shaped identically to the live one, because
that is the only reason it works.

Two deliberate choices worth knowing:

- **A failed copy raises** rather than letting the submission continue. Continuing past a
  failed archive reproduces exactly this bug; a blocked launch is recoverable, destroyed
  measurements are not.
- **Inputs are not copied.** `code.tar.gz` and `manifest.json` are regenerated by every
  submission and the tarball is megabytes; archiving them would slow every resubmission
  for nothing recoverable.

**Still true, and not fixed here:** the live prefix is still last-write-wins, so a
resubmission *does* replace what a reader sees at `pipeline-runs/<run_id>/`. The change is
that the predecessor now survives beside it instead of being destroyed.

**Before the fix:** before resubmitting a manifest whose earlier run produced results
