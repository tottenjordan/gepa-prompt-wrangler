"""Run GEPA optimization for all 5 models sequentially from generic 78-char prompt.

Uses the expanded 5-criteria sampler config (response_match, final_response_match_v2,
safety, rubric_based_final_response_quality_v1, rubric_based_tool_use_quality_v1).
"""
...
def main():
    ...
    print(f"{'=' * 60}")
    print(f"GEPA OPTIMIZATION — ALL MODELS (v4, 5 criteria)")
    print(f"{'=' * 60}")
    print(f"  Models:   {', '.join(MODELS)}")
    print(f"  Seed:     generic 78-char prompt")
    print(f"  Criteria: response_match, final_response_match_v2, safety,")
    print(f"            rubric_response_quality (2 rubrics),")
    print(f"            rubric_tool_use_quality (2 rubrics)")
    print(f"  Eval:     {EVAL_PATH} (40 cases, 28 train / 12 val)")
    print()
