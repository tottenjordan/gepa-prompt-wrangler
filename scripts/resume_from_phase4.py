"""Resume the experiment from Phase 4 using saved optimization results."""
...
def _load_optimized_prompt(pair_id: str) -> str:
    """Load the wrangler_v3 optimized prompt from the agent's prompts file."""
    stem = pair_id.split("-")[0]
    prompts_file = Path("examples/multi_model_agents/prompts") / f"{stem}_prompts.py"
    import ast
        ...
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "OPTIMIZED":
                    optimized = ast.literal_eval(node.value)
                    if "wrangler_v3" in optimized:
                        return optimized["wrangler_v3"]["prompt"]
    raise ValueError(f"wrangler_v3 not found in {prompts_file}")
...
def main():
    ...
    print(f"{'=' * 60}")
    print(f"GEPA PROMPT WRANGLER — RESUMING FROM PHASE 4")
    print(f"{'=' * 60}")
    ...
    for pair in pipeline.manifest.pairs:
        optimized = _load_optimized_prompt(pair.id)
        pair.system_prompt = optimized
        pipeline.results[pair.id] = {
            "model": pair.model,
            "original_prompt": "You are a helpful assistant. Use the available tools to answer user questions.",
            "engine_id": pair.engine_id,
            "before": BASELINE_SCORES[pair.id],
            "optimized_prompt": optimized,
        }
        print(f"  [{pair.id}] Loaded wrangler_v3 prompt ({len(optimized)} chars)")
