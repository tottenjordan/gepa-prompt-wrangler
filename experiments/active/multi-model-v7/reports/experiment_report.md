# GEPA Prompt Wrangler — multi-model-v7

## Executive Summary

**0/5 models improved** after GEPA optimization. 
Largest regression: **sonnet** (-0.134 avg). 

- **Regressed:** lite (-0.042), flash (-0.024), pro (-0.050), sonnet (-0.134), opus (-0.102)

**Largest metric decline:** Response Match (-0.223 avg across models)

## Methodology

**Experiment:** `multi-model-v7`
...
| Metric |Lite | Flash | Pro | Sonnet | Opus |
|--------|------ | ------ | ------ | ------ | ------ |
| Response Quality | 0.76 ±0.03 | 0.82 ±0.02 | 0.82 ±0.03 | 0.77 ±0.17 | 0.81 ±0.02 |
| Hallucination | 0.97 ±0.01 | 0.95 ±0.01 | 0.99 ±0.01 | 0.89 ±0.04 | 0.89 ±0.04 |
| Safety | 0.99 ±0.01 | 0.99 ±0.01 | 0.98 ±0.01 | 0.96 ±0.03 | 0.96 ±0.01 |
| Tool Use | 0.44 ±0.05 | 0.39 ±0.03 | 0.44 ±0.02 | 0.36 ±0.04 | 0.37 ±0.01 |
| Instruction Following | 0.52 ±0.03 | 0.67 ±0.07 | 0.46 ±0.11 | 0.46 ±0.04 | 0.54 ±0.01 |
| Response Match | 0.56 ±0.04 | 0.56 ±0.05 | 0.44 ±0.18 | 0.26 ±0.04 | 0.28 ±0.04 |

### Improvement Delta (After - Before)

| Metric |Lite | Flash | Pro | Sonnet | Opus |
|--------|------ | ------ | ------ | ------ | ------ |
| Response Quality | -0.07 | +0.00 | +0.00 | -0.07 | -0.03 |
| Hallucination | +0.01 | -0.02 | +0.02 | -0.04 | +0.00 |
| Safety | -0.00 | +0.00 | -0.00 | -0.01 | +0.03 |
| Tool Use | +0.00 | -0.07 | -0.00 | -0.06 | -0.05 |
| Instruction Following | -0.10 | +0.00 | -0.14 | -0.21 | -0.19 |
| Response Match | -0.09 | -0.07 | -0.18 | -0.40 | -0.37 |
| **Average** | **-0.04** | **-0.02** | **-0.05** | **-0.13** | **-0.10** |

## Per-Model Analysis

### Lite (`gemini-3.1-flash-lite`, $1.75/M)

**Overall:** 0.75 → 0.71 (-0.042, regressed)

- **Lost:** Response Quality, Instruction Following, Response Match
- **Prompt expansion:** 78 → 1342 chars (17x)

### Flash (`gemini-3.5-flash`, $10.50/M)

**Overall:** 0.75 → 0.73 (-0.024, regressed)

- **Lost:** Hallucination, Tool Use, Response Match
- **Prompt expansion:** 78 → 4249 chars (54x)

### Pro (`gemini-3.1-pro-preview`, $22.00/M)

**Overall:** 0.74 → 0.69 (-0.050, regressed)

- **Gained:** Hallucination
- **Lost:** Instruction Following, Response Match
- **Prompt expansion:** 78 → 4139 chars (53x)

### Sonnet (`claude-sonnet-4-6`, $18.00/M)

**Overall:** 0.75 → 0.62 (-0.134, regressed)

- **Lost:** Response Quality, Hallucination, Safety, Tool Use, Instruction Following, Response Match
- **Prompt expansion:** 78 → 1996 chars (26x)

### Opus (`claude-opus-4-6`, $30.00/M)

**Overall:** 0.74 → 0.64 (-0.102, regressed)

- **Gained:** Safety
- **Lost:** Response Quality, Tool Use, Instruction Following, Response Match
...
|-------|-------|-----------|--------|-------|-------|----------|
| Lite | `gemini-3.1-flash-lite` | $1.75 | 0.75 | 0.71 | -0.04 | 0.404 |
| Flash | `gemini-3.5-flash` | $10.50 | 0.75 | 0.73 | -0.02 | 0.069 |
| Pro | `gemini-3.1-pro-preview` | $22.00 | 0.74 | 0.69 | -0.05 | 0.031 |
| Sonnet | `claude-sonnet-4-6` | $18.00 | 0.75 | 0.62 | -0.13 | 0.034 |
| Opus | `claude-opus-4-6` | $30.00 | 0.74 | 0.64 | -0.10 | 0.021 |
...
1. **Investigate Response Match regression** (-0.223 avg in regressed models) — consider adding as explicit optimization target in sampler config
