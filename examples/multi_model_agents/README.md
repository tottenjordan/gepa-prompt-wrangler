...
├── eval_data/                 # Eval cases (64 cases across 3 complexity tiers)
│   ├── eval_cases.yaml
...
- `wrangler-{name}-agent-{version}` display name (e.g., `wrangler-lite-agent-v4`)
...
Run the full 64-case eval dataset against deployed agents:
...
64 eval cases across 3 complexity tiers:

| Tier | Cases | Description |
|------|-------|-------------|
| **Low** | 33 | Single tool call — flight search, hotel search, policy checks, booking, expense history, edge cases (invalid codes, unknown categories), boundary-value policy checks (exact limit, one cent over), error recovery (non-existent bookings, unknown users) |
| **Medium** | 22 | 2 tools — submit with policy check, flight comparison, multi-category policy, search + policy cross-check, book + verify, savings analysis, book + cancel flow, multi-boundary checks, no-match route handling |
| **High** | 9 | 3+ tools — book + policy + expense pipeline, multi-route comparison with hotels, budget-constrained planning, multi-user audit, end-to-end trip booking |
...
| Agent Engine | `wrangler-lite-agent-{version}` | Reasoning Engine |
| Agent Engine | `wrangler-flash-agent-{version}` | Reasoning Engine |
| Agent Engine | `wrangler-pro-agent-{version}` | Reasoning Engine |
| Agent Engine | `wrangler-sonnet-agent-{version}` | Reasoning Engine |
| Agent Engine | `wrangler-opus-agent-{version}` | Reasoning Engine |
